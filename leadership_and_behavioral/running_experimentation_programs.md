#  running experimentation programs.Value.ToUpper() unning  running experimentation programs.Value.ToUpper() xperimentation  running experimentation programs.Value.ToUpper() rograms

## Definition
-

---

## When It Appears in Interviews
-

---

## Core Ideas Known Cold
-

---

## How to Answer (Framework)
1. Clarify the goal and metric
2. Identify assumptions and constraints
3. Explain the approach
4. Call out pitfalls
5. Describe validation

---

## Common Pitfalls
-

---

## Mini Example
-

---

## Practice Questions
-

---

## Checklist
- [ ] I can explain this verbally
- [ ] I know alternatives
- [ ] I know failure modes
